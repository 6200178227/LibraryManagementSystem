...
color:blue;
    ...
    <body>
         <header>
                  <ul style="float: left">
                      <li style="color:white;font-size: 20px"> <b>Returned Book Details</b></li>
            </ul>
<div >
        ...
        <table class="table table-bordered table-striped table-hover" >
            ...
            <tbody>
                <%
                    ...
                    if(data==null)
                           {
                     s="select * from returnbook";
                            }
                   else
                   {
                       s="select * from returnbook where return_id like '%"+data+"%' or mem_id like '%"+data+"%' or mem_name like '%"+data+"%' or acc_no like '%"+data+"%' or lib_id  like '%"+data+"%' or mem_type like '%"+data+"%' or issue_date like '%"+data+"%' or return_date like '%"+data+"%' or fine like '%"+data+"%'";
                   }
                    r=p.executeQuery(s);
                    while(r.next())
                    { 
                %>
                    <tr>
                      ...
                      <td><%=r.getString("return_date")%></td> 
                      <td><%=r.getString("fine")%></td> 
                      
                      
                       <td>
                           <a href="deletereturndetails.jsp?u1=<%=r.getString("return_id")%>" class="btn btn-warning">Delete</a>
                       </td>
                    </tr>
                    <%
                        }
